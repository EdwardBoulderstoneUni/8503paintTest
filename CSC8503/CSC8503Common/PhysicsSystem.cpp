...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) {
	...
	{
		if (a.GetTrigger() && a.GetTag() == "Slower")//Check both name and Trigger
		{
			SlowenObjectResolveCollision(b);
		}
		else if (a.GetTrigger() && a.GetName() == "FastenSphere")
		{
			FastenObjectResolveCollision(b);
		}
		else if (a.GetTrigger() && a.GetTag() == "Booster")
		{
			BoosterCubeResoloveCollision(a, b, p);
		}
		else if (a.GetTrigger() && a.GetTag() == "KillFloor")
		{
			Debug::Print("You Died!!!!!! Press F1 to restart", Vector2(50, 50));
			b.GetPhysicsObject()->SetLinearVelocity(Vector3(0, 0, 0));
		}
		else if (a.GetTrigger() && a.GetName() == "WinBroad")
		{
			Debug::Print("You Win!!!!!! ",Vector2(20,50));
			b.GetPhysicsObject()->SetLinearVelocity(Vector3(0, 0, 0));
		}
		return;
	}

	if (a.GetName() == "Player" && b.GetName() == "Bonus")
	{
		GetScore();
	}
	

	if ( a.GetName() == "StartCube2"&&a.GetTag() == "Start2")
	{
		b.GetPhysicsObject()->SetLinearVelocity(Vector3(0, 0, -40));
	}


	if (a.GetName() == "StartCube" && a.GetTag() == "Start")
	{
		b.GetPhysicsObject()->SetLinearVelocity(Vector3(500, 0, 30));
		b.GetPhysicsObject()->SetAngularVelocity(Vector3(50, 50, 50));
	}
	...
	physA->SetElasticity(0.66f);
	physB->SetElasticity(0.66f);
...
void PhysicsSystem::KillFloorResolveCollision(GameObject& objecyt)
{
}

void PhysicsSystem::SlowenObjectResolveCollision(GameObject& object)//LinerVelocity reducer
{
	//object.GetPhysicsObject()->SetLinearVelocity(Vector3(-5, 0, 0));
	object.GetPhysicsObject()->AddForce(Vector3(50, 0, 0));
}

void PhysicsSystem::FastenObjectResolveCollision(GameObject& object)//LinerVelocity booster
{
	object.GetPhysicsObject()->AddForce(Vector3(500, 0, 0));
}

void PhysicsSystem::BoosterCubeResoloveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p)
{
	Vector3 forceDir = p.normal;
	Vector3 forceFactor = forceDir*20.0f;


	b.GetPhysicsObject()->ApplyLinearImpulse(forceFactor);
	b.GetPhysicsObject()->ApplyAngularImpulse(forceFactor);
}


//Spring collision(My) constraitint
void PhysicsSystem::ResolveSpringCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p)
{
	...
	// Separate attributes out using projection
	transformA.SetPosition(transformA.GetPosition() -
		(p.normal * p.penetration * (physA->GetInverseMass() / totalMass)));
	transformB.SetPosition(transformB.GetPosition() +
		(p.normal * p.penetration * (physB->GetInverseMass() / totalMass)));
	...
	Vector3 force = p.normal *p.penetration * physA->GetElasticity();
...
//Tempoary constraint:change color to blue when collide ,red after collide
void PhysicsSystem::ResolutionConstraint(GameObject& a,GameObject& b,CollisionDetection::ContactPoint& p)
{
...
//Check everything and store it in tree
...
for (auto i = first; i != last; ++i)
{
	...
	if (object->GetTriggerValue() == true)
	{
		continue;
	}
